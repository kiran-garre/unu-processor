 ...
 * 
 * efree():
 * 		Although it looks like a long function, the logic is mostly the same
 * 		is insertion into a linked list, with some more cases handled when
 * 		free sections are perfectly adjacent. The linked list is stored as 
 * 		headers in the memory blocks themselves and can be overwritten after 
 * 		being allocated.
 * 		
